...
from stock_data.db import connect as stock_connect
...
def connect(
    ...
    connection = stock_connect(db_path)
    if market_db is None:
        return connection
    market_path = Path(market_db).resolve()
    connection.execute("ATTACH DATABASE ? AS market_source", (str(market_path),))
    connection.execute("PRAGMA market_source.query_only=ON")
    tables = (
