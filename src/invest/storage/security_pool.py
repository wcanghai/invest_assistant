...
def init_db(connection: sqlite3.Connection, schema_path: str | Path | None = None) -> None:
    ...
    path = Path(schema_path) if schema_path else Path(__file__).with_name("security_pool.sql")
...
def save_etf_snapshot(
    ...
    for row in rows:
        ...
        snapshot_rows.append({
            ...
            **{column: row.get(column) for column in ETF_COLUMNS
               if column not in {"data_date", "updated_at"}},
