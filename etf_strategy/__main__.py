"""Run python -m etf_strategy --help for the complete local workflow."""
...
from .config import LEGACY_STRATEGIES, STRATEGIES, load_config
from .data import Dataset, audit
from .manual import import_account_events, import_fills, import_snapshot, reconcile, trade_plan
from .storage import clean, import_records, migrate, save_run
...
def parser():
    ...
    cli.add_argument("--db", default="data/security_pool.db")
    cli.add_argument("--results-db", default="data/etf_strategy.db")
...
def main():
    ...
    if command == "full-market" or (command == "compare" and cfg["version"] == "2.0.0"):
        from .full_market import run_suite

        run_suite(args.db, args.output, args.start, args.end or "2026-09-11")
        return
    if command == "migrate":
        migrate(args.db)
    ...
    run_id, result = save_run(args.results_db, command, cfg, result)
