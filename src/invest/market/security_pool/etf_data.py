...
from invest.providers.stock import to_tdx_date
from invest.market.stock.processor import flatten_daily_bars

from invest.storage import security_pool as db
from invest.providers.security_pool import call_with_retry
from invest.providers.security_pool import init_tdx
from invest.market.security_pool.processor import clean_name
...
def fetch_etf_master_rows(client: Any, data_date: str) -> list[dict[str, Any]]:
    ...
    for index, (etf_code, category_name) in enumerate(sorted(members.items()), start=1):
        ...
        rows.append({
            ...
            "raw_info_json": json.dumps(dict(info), ensure_ascii=False,
...
def upsert_etf_daily(connection: sqlite3.Connection, rows: Sequence[Mapping[str, Any]]) -> int:
    ...
    for column in ETF_DAILY_COLUMNS:
        ...
        if column == "raw_metrics_json":
            updates.append(
                "raw_metrics_json=json_patch(COALESCE(etf_daily.raw_metrics_json,'{}'),"
                "excluded.raw_metrics_json)"
            )
    ...
    try:
        ...
        with connection:
            connection.execute(
                ...
                "current_price,previous_close,iopv,shares_10k,size_100m,"
                "premium_discount_pct,archived_at) "
                "SELECT data_date,index_code,etf_code,etf_name,current_price,previous_close,iopv,"
                "shares_10k,size_100m,CASE WHEN iopv<>0 THEN "
                "(current_price/iopv-1)*100 END,updated_at "
                "FROM etf_info"
            )
